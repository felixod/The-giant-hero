...
SELECT @columns = STUFF(
    (
        ...
        FROM (
            SELECT id FROM MSCADA.dbo.MSPDB_Params WHERE CHARINDEX(',' + CAST(id AS VARCHAR(10)) + ',', ',' + @selected_ids + ',') > 0
        ) AS filtered_ids
        FOR XML PATH(''), TYPE
    ).value('.', 'NVARCHAR(MAX)'), 
    1, 2, ''
...
WITH DataPrep AS (
    ...
    WHERE CHARINDEX('','' + CAST(p.id AS VARCHAR(10)) + '','', '','' + @selected_ids + '','') > 0 AND h.Dtm BETWEEN @start_date AND @end_date
)
SELECT * FROM
(
