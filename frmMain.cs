...
namespace SQLBuilder
{
	...
	public partial class frmMain : Form
	{
		...
		{
			public static List<Department> LoadDepartments(string iniFilePath)
			{
				List<Department> departments = [];
				IniFile ini = new(iniFilePath);
				LoadDepartment(ini, "General0", departments);
				return departments;
			}

			private static void LoadDepartment(IniFile ini, string section, List<Department> departments)
			{
				Department department = new()
				{
					Name = ini.ReadKey(section, "Name")
				};

				string s = ini.ReadKey(section, "SubDepartmentCount");
				int subDepartmentCount = int.Parse(s);

				for (int i = 1; i <= subDepartmentCount; i++)
				{
					string subDepartmentSection = section + ".SubDepartment" + i;
					LoadDepartment(ini, subDepartmentSection, department.SubDepartments);
		...
		private void frmMain_Load(object sender, EventArgs e)
		{
			...
			// Загружаем дерево
			DepartmentLoader loader = new();
			List<Department> departments = DepartmentLoader.LoadDepartments("departments.ini");

			treeView.Nodes.Clear();
			AddDepartmentsToTreeView(departments, treeView.Nodes);
		...
		static void AddDepartmentsToTreeView(List<Department> departments, TreeNodeCollection nodes)
		{
			foreach (var department in departments)
			{
				TreeNode node = new(department.Name);
				nodes.Add(node);
				AddDepartmentsToTreeView(department.SubDepartments, node.Nodes);
			}
		}
		...
		private void frmMain_FormClosed(object sender, FormClosedEventArgs e)
		{
			...
			IniFile ini = new("departments.ini");
			SaveTreeViewToIni(treeView.Nodes, ini, "General", 0);

		}


		static void SaveTreeViewToIni(TreeNodeCollection nodes, IniFile ini, string section, int level)
		{
			foreach (TreeNode node in nodes)
			{
				string currentSection = section + level;
				ini.WriteKey(currentSection, "Name", node.Text);

				if (node.Nodes.Count > 0)
				{
					ini.WriteKey(currentSection, "SubDepartmentCount", node.Nodes.Count.ToString());
					SaveTreeViewToIni(node.Nodes, ini, currentSection + ".SubDepartment", 1);
				}
				else
				{
					ini.WriteKey(currentSection, "SubDepartmentCount", "0");
				}
		...
		private void deleteToolStripMenuItem_Click(object sender, EventArgs e)
		{
			if (treeView.SelectedNode != null)
			{
				...
				IniFile ini = new IniFile("departments.ini");

				string section = GetSectionFromNode(selectedNode);
				ini.DeleteKey(section, "Name");
				ini.DeleteKey(section, "SubDepartmentCount");
		...
		static string GetSectionFromNode(TreeNode node)
		{
			string section = "";
			...
			while (currentNode != null)
			{
				if (section == "")
				{
					section = currentNode.Text;
				}
				else
				{
					section = currentNode.Text + "." + section;
				}
