...
namespace SQLBuilder
{
	...
	public partial class frmMain : Form
	{
		...
		private void OpenSensorForm(bool insert)
		{
			if (treeView.SelectedNode != null)
			{
				frmSensor form = new(insert, (int)treeView.SelectedNode.Tag);
				form.ShowDialog();
			}
			LoadSensors();
		}
		...
		private void LoadSensorForIdXML(string xmlFilePath, int nodeId)
		{
			...
			if (File.Exists(xmlFilePath))
			{
				...
				if (node != null)
				{
					...
					{
						var param = new ListViewItem(new[] { sensorNode.Attributes["Name"].Value,
															 sensorNode.Attributes["Type"].Value,
															 sensorNode.Attributes["Description"].Value,
															 sensorNode.Attributes["Formula"].Value })
						{
							Tag = nodeId
						};
